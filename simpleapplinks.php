...
class SimpleAppLinks {
    ...
    function enqueue_script() {
        wp_register_script('simpleapplinks', plugins_url('simpleapplinks.js', __FILE__), array('jquery'), NULL, true);
...
<?php 
    ...
    function get_applink($url) {
        ...
        if ($cache === null || ($cache !== null && $cache->updated_at < date('Y-m-d H:i:s', time() - $this->options['cache-time']))) {
            ...
            case 'itunes.apple.com':
                ...
                break;
            case 'wordpress.org':
                $data = $this->parse_wordpress_plugin($dom, $html);
                break;
    ...
    function parse_android_app($dom) {
        ...
        $reviewNum = $dom->find('.EymY4b', 0);
        $score = $dom->find('.BHMmbe', 0);
    ...
    function parse_ios_app($dom) {
        $title = $dom->find('h1.product-header__title', 0)->first_child()->innertext = '';
        $title = $dom->find('h1.product-header__title', 0);
        $supplier = $dom->find('h2.product-header__identity', 0);
        $_supplier = explode(' ', $supplier->plaintext);
        unset($_supplier[0]);
        $icon  = $dom->find('div.product-hero__media img', 0);
        $iconUrl = $icon->getAttribute('src');
        //$price = $dom->find('div.price', 0);
		$price = '';
        $_price = $dom->find('li.product-header__list__item');
		$price = $_price[count($_price)-1];
        ...
        $_review_count = $dom->find('h4.we-customer-ratings__count', 0);
		$review_count = '0';
		if ($_review_count) {
			preg_match('/[0-9\.K]+/', $_review_count->innertext, $match);
			$review_count = $match[0];
		}
        $rating = $dom->find('span.we-customer-ratings__averages__display', 0);
        // rating is not in html when review is a few
        $score = ($rating != null) ?  $this->rating_star(intval($rating->innertext())): '';
        //$score = '';
    ...
    function parse_wordpress_plugin($dom, $html) {
        $title = $dom->find('h1.plugin-title', 0);
        $supplier = $dom->find('.plugin-contributor-info a', 0);

        // get icon
        $iconUrl = plugins_url('images/wordpress-logo.png', __FILE__);
        $url = $dom->find('link[rel=canonical]', 0);
        ...
        preg_match('/"dateModified": "([0-9\-:T\+)*",/', $html, $matches);
        $published = (count($matches) > 1) ? $matches[1] : 0;
        preg_match('/"ratingCount": ([0-9]*),/', $html, $matches);
        $reviewNum = (count($matches) > 1) ? $matches[1] : 0;
        preg_match('/"ratingValue": "([0-9]*.?[0-9]*)"/', $html, $matches);
        $score = (count($matches) > 1) ? $matches[1] : 0;
        ...
        $ret = array(
            ...
            'score'       => $this->rating_star($score),
            'published'   => $published,
            'download'    => 'NULL'#$_download[1],
        );
        return $ret;
    }
    ...
    function parse_chrome_extension($dom) {
        ...
        $_download = $download ? explode(':', $download->getAttribute('content')) : array(0,0);
